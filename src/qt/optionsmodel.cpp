...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::VRZ);
    ...
    if (!settings.contains("nAnonymizeVrzAmount"))
        settings.setValue("nAnonymizeVrzAmount", 1000);

    nPrivatesendRounds = settings.value("nPrivatesendRounds").toLongLong();
    nAnonymizeVrzAmount = settings.value("nAnonymizeVrzAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVrzAmount"))
        SoftSetArg("-anonymizevrzamount", settings.value("nAnonymizeVrzAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in vrz.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVrzAmount:
            return QVariant(nAnonymizeVrzAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivatesendRounds:
            ...
            break;
        case AnonymizeVrzAmount:
            nAnonymizeVrzAmount = value.toInt();
            settings.setValue("nAnonymizeVrzAmount", nAnonymizeVrzAmount);
            emit anonymizeVrzAmountChanged(nAnonymizeVrzAmount);
            break;
        case CoinControlFeatures:
