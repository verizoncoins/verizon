...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VRZ)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("VRZ")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivatesendProgress()
{
    ...
    QString strAnonymizeVrzAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVrzAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeVrzAmount = strAnonymizeVrzAmount.remove(strAnonymizeVrzAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVrzAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeVrzAmount * COIN) nMaxToAnonymize = nAnonymizeVrzAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeVrzAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeVrzAmount));
        strAnonymizeVrzAmount = strAnonymizeVrzAmount.remove(strAnonymizeVrzAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVrzAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    } else {
