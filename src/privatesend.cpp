    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Verizon and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 VRZ per transaction on average.
            */
            if (r <= 10) {
...
bool CPrivatesendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeVrzAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CPrivatesendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100VRZ+1 ( bit on if present )
    // bit 1 - 10VRZ+1
    // bit 2 - 1VRZ+1
    // bit 3 - .1VRZ+1
    // bit 3 - non-denom
