...
_Setup instructions for a gitian build of Verizon using a Debian VM or physical system._

Gitian is the deterministic build process that is used to build the Verizon
...
- Disk size: at least 40GB; as low as 20GB _may_ be possible, but better to err on the safe side
...
Clone the git repositories for vrz and gitian and then checkout the vrz version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/vrz-crypto/vrz.git
cd vrz
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vrz repository
...
To build Verizon (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vrz repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vrz/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/vrz-crypto/vrz
...
```bash
URL=https://github.com/crowning-/vrz.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit vrz=${COMMIT} --url vrz=${URL} ../vrz/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vrz=${COMMIT} --url vrz=${URL} ../vrz/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vrz=${COMMIT} --url vrz=${URL} ../vrz/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vrz-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vrz-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vrz-build.assert
```
...
[vrz/gitian.sigs](https://github.com/vrz-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
