Verizon Core version 2.3.0 is now available from:
...
| `outputs`         | Result is now an array of objects instead of a list of _n_ objects                                                           |
| `status`          | Added additional fields for txhash, outputidx, netaddr, and message                                                          |
| `winners`         | Result is now an array of objects instead of a list of _n_ objects. See below                                                |
...
[
  {
    ...
    winner: {
        address: addr,    (string) Verizon MN Address,
...
[
  {
    ...
    "addr": addr,      (string) Masternode Verizon address
...
Verizon Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for Verizon (Mrs-X)
...
- #191 `3a778c3` [Tests] Fix the unit test suite for use with Verizon (Fuzzbawls)
